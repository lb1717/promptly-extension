...
async function cmdAlignDevice(flags) {
  ...
  if (sourceUids.length) {
    try {
      ...
      console.log(
        `Consolidated historical stats: ${consolidation.eventsMoved ?? consolidation.events_moved ?? 0} events, ${consolidation.devicesMoved ?? consolidation.devices_moved ?? 0} devices → ${consolidation.target_email || primary.email || primary.uid}`
      );
    } catch (err) {
